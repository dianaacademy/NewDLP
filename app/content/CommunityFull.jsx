...
import { getFirestore, collection, query, orderBy, limit, startAfter, getDocs, doc, updateDoc, increment, addDoc, serverTimestamp, setDoc, getDoc, onSnapshot } from 'firebase/firestore';
  ...
  const handlePostReply = async () => {
    ...
    try {
      const replyData = {
        ...
        reply_by: auth.currentUser?.displayName || "Current User",
        profile_reply: auth.currentUser?.photoURL || "https://firebasestorage.googleapis.com/v0/b/ddlp-456ce.appspot.com/o/images%2F11.png?alt=media&token=4d97471b-c547-4c80-94ab-d8592669c5df"
      };
...
const CommunityFull = () => {
  ...
  useEffect(() => {
    if (auth.currentUser) {
      const userLikesRef = doc(db, 'user_likes', auth.currentUser.uid);
      const unsubscribe = onSnapshot(userLikesRef, (doc) => {
        if (doc.exists()) {
          const likedPostsSet = new Set(doc.data().likedPosts);
  ...
  const handleLikePress = async (postId) => {
    try {
      ...
      let likedPostsSet = new Set(userLikesDoc.exists() ? userLikesDoc.data().likedPosts : []);
      ...
      // Update Firestore with the new liked posts
      ...
      setPosts(prev => prev.map(post =>
        post.id === postId
          ? { ...post, post_like: post.post_like + (likedPostsSet.has(postId) ? 1 : -1) }
          : post
      ));
    } catch (error) {
  ...
  const renderPost = ({ item }) => (
    <View style={styles.postContainer}>
      <Image
        source={{ uri: item.post_by_profile_URL || "https://firebasestorage.googleapis.com/v0/b/ddlp-456ce.appspot.com/o/images%2F11.png?alt=media&token=4d97471b-c547-4c80-94ab-d8592669c5df" }}
